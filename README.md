...
software. In the future as log parser matures, new capibility may be added to make it work with other tools. For now,
it is assumed that the user has access to telemetry logs generated by the airliner's `ds` application.

At the moment only telemetry is known to work.

Commands parsing is still under review.
...
Documentation updated on October 23, 2020
