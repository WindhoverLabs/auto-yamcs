...
def parse_cli() -> argparse.Namespace:
    ...
    """
    base_parser = argparse.ArgumentParser(description='Takes in path to sqlite database.', add_help=False)

    base_parser.add_argument('--juicer_mode', type=str, default='SQLITE', choices=['SQLITE'],
                             help='The mode which to run juicer on')

    base_parser.add_argument('--output_file', type=str, default='newdb.sqlite',
                             help='The output file juier will write to.', required=True)

    base_parser.add_argument('--verbosity', type=str, default='0', choices=['0', '1', '2', '3', '4'],
    ...
    # inline_parser.add_argument('--output_file', type=str, default='newdb.sqlite', help='The output file juier will write to.')

    # inline_parser.add_argument('--verbosity', type=str, default='0', choices=['0', '1', '2', '3', '4'],
    #                            help='[(0=SILENT), (1=ERRORS), (2=WARNINGS), (3=INFO), (4=DEBUG)]')
...
def main():
    ...
    # FIXME: Not the cleanest way of handling the modes.
    if len(sys.argv) <= 1:
        print('Must pass a mode {inline, singleton}')
        return -1

    elif sys.argv != 'inline' or sys.argv != 'singleton':
        print('Must pass a mode {inline, singleton}')
        return -1
