...
def merge(source: dict, destination: dict):
    """
    Overwrites the contents of destination dictionary with the contents of source. Beware that this is done in-place;
    the destination dictionary is modified.
    :param source:
    :param destination:
    :return:
    """
    ...
    if args.schema:
        ...
        if is_source_valid is True:
            ...
            if is_destination_valid is False:
                logging.info(f'There was a problem validating "{args.destination}".')

        else:
            logging.info(f'There was a problem validating "{args.source}".')
